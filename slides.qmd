...
subtitle: "LLM-powered classification in R 🪄"
...
:::
::: {.column width="40%"}
### Use [`library(ellmer)`](https://ellmer.tidyverse.org/) robustly for delicate use cases  {.fragment .fade-in-then-semi-out} 
...
::: {.incremental}
-   **Accuracy**: `r accuracy`% (vs. 93% in tidymodels)
...
certainty_plot |>
  ...
  ggplot2::labs(
    ...
    subtitle = glue::glue("Higher certainty → lower error rates (r = {round(correlation, 3)})"),
    x = "LLM certainty",
...
#| echo: true
chat <- chat("openai/gpt-5-mini")
chat$chat("Tell one joke about R")
```
:::
:::
...
-   Use **uncertainty scores** and **human review**
