...
mean(oai_dat$diagnosis == symptoms$label) # tidymodels: 93% accuracy
...
Agreement with [ML tool](https://cjars-toc.isr.umich.edu): 80-82%
...
ml_confident <- combined_data |>
  dplyr::filter(
    ...
    toc_uncertainty <= 0.1 & llm_uncertainty >= 0.5
  ) |>
...
llm_confident <- combined_data |>
  dplyr::filter(
    toc_offense_type != openai_offense_type & 
    llm_uncertainty <= 0.1 & toc_uncertainty >= 0.5
  ) |>
  dplyr::slice_head(n = 1)
...
both_cases <- dplyr::bind_rows(ml_confident, llm_confident) |>
  dplyr::mutate(
    case_name = c("ML Confident", "LLM Confident"),
    description_short = ifelse(nchar(description) > 40, 
                              paste0(substr(description, 1, 40), "..."), 
                              description)
  ) |>
  dplyr::select(case_name, description_short, toc_offense_type, toc_uncertainty,
                openai_offense_type, llm_uncertainty)

both_cases |>
  knitr::kable(
    col.names = c("Case", "Description", "ML Tool", "ML?", "LLM", "LLM?"),
    digits = 3
  )
```
:::
